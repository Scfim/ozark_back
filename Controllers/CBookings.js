      ...
      if (
        ...
        if (
          ...
          Reference.getCount((resultRefe) => {
            if (
              resultRefe.data.type === "success" &&
              resultRefe.data.data.length > 0
            ) {
              if (
                resultRefe.data.data.booking_reference_number !== null &&
                resultRefe.data.data.booking_reference_number !== "" &&
                resultRefe.data.data.booking_reference_number !== undefined
              ) {
                const refNumber =
                  resultRefe.data.data.booking_reference_number + 1;
                Reference.insert(
                  {
                    number: refNumber,
                    date: dateRecord,
                      ...
                      if (paymentCheck.check === true) {
                        if (verifyBooking === true) {
                          Payment.insert(
                            ...
                            (resultPayment) => {
                              if (resultPayment.type !== "success")
                                response.send({
                                  type: "failure",
                                  message: "Echec d'enregistrement du paiement",
                                });
                            }
                          );
                        }
                        ...
                        Booking.get(
                          ...
                          (resultBooking) => {
                            if (resultBooking.data.type === "success") {
                                  ...
                                  (resultOutput) => {
                                    if (resultOutput.data.type !== "success") {
                                      response.send({
                                        type: "failure",
                                        message:
                                          "Echec d'enregistrement de la sortie numero " +
                                          data[i],
                                      });
                                      return;
                                    } else {
                              ...
                              response.send({
                                type: "failure",
                                message: "Echec de recuperation de la commande",
                              });
                          }
                        );
                      }
