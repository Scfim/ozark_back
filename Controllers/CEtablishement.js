...
routes.post('/update',sessionHandler,(request,response)=>{
  ...
  if(request.session.user){
      ...
      if(validator(field).isString().check()===true){
        if(validator(value).isString().check()===true){
          if(field==="Mail" ){
            ...
            }else response.send({type: "failure", message: `Le site  web est invalide`});
          }
          else if(field==="bp"){
            if( validator(value).isString().check()===true){
              User.update({
            ...
            }else response.send({type: "failure", message: `La boite postal`});
          }
    ...
    }else response.send({type: "failure", message: `le nomdoit etre un text`});
